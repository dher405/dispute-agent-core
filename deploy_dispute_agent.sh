...
echo "================================================================="
echo "   Dispute Agent Platform: Schema Migration & Deployment Engine   "
echo "================================================================="
...
if [ -z "${DATABASE_URL:-}" ]; then
    echo "[-] ERROR: DATABASE_URL is still missing."
    exit 1
fi

echo "[+] Applying multi-vertical DDL migrations to dispute_db_f372..."

psql "$DATABASE_URL" << 'EOSQL'
-- Add ISP/telecom & multi-vertical attributes
ALTER TABLE leads
...
-- Table for tracking carrier and ISP response events
...
-- Staged leads view
CREATE OR REPLACE VIEW v_staged_leads_for_review AS
SELECT 
    id::text AS id,
    vertical,
    source_platform,
    ...
    created_at
FROM leads
WHERE status = 'staged_for_review'
ORDER BY created_at DESC;
EOSQL

echo "[+] Database schema and views successfully updated."

# Write requirements.txt
cat << 'REQS_EOF' > requirements.txt
fastapi>=0.115.0
...
git add requirements.txt main.py deploy_dispute_agent.sh
git commit -m "feat(dispute-core): enable multi-vertical statutory intake and webhook reconciliation" || echo "No changes to commit."
...
echo "================================================================="
echo "   Deployment Complete and Pushed to Render!                     "
echo "   - API Base:    https://dispute-api-xyl7.onrender.com          "
echo "   - Inbound Hook: https://dispute-api-xyl7.onrender.com/api/v1/webhooks/carrier/inbound "
echo "================================================================="
